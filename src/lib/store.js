import { writable, derived, get as take } from "svelte/store";
import * as countries_raw from "$lib/data/countries_with_wikidata.json";
...
export const DELIMITER = writable(
  initValue(DELIMITERS[0].value, "cg-option-delimiter"),
);
DELIMITER.subscribe((value) => safeValue("cg-option-delimiter", value));

// Include Input?
export const INCLUDE_INPUT = writable(
  initValue(false, "cg-option-include-input"),
);
INCLUDE_INPUT.subscribe((value) => safeValue("cg-option-include-input", value));

// Include Name?
export const INCLUDE_NAME = writable(
  initValue(false, "cg-option-include-name"),
);
...
export const OUTPUT = writable(
  initValue(getOptionValue(OUTPUT_OPTIONS[0]), "cg-option-output"),
);
...
const miniSearch = new MiniSearch({
  ...
  storeFields: [
    KEY_LABEL,
    KEY_OFFICIAL,
    ...OUTPUT_OPTIONS.map((o) => getOptionValue(o)),
...
export const SPLIT_CHAR = writable(
  initValue(getOptionValue(SPLIT_CHARS[0]), "cg-option-split-char"),
);
...
export const OPTIONS = derived(
  [UNIQUE_INPUT, MATCHES, CUSTOM],
  ([input, matches, custom]) => {
...
export const PAIRS = derived(
  [INPUT, MATCHES, CUSTOM, OUTPUT],
  ([input, matches, custom, ouput]) => {
    return input.map((datum) => {
